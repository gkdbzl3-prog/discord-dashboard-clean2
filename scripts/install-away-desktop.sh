...
# 창 클래스로 잡는다. 제목으로 잡으면 빗나간다 — 크롬 창이 처음 뜰 때의
# 제목은 아직 페이지의 <title> 이 아니고, KWin 은 그 순간에 규칙을 본다.
# 실제로 그렇게 한 번 놓쳤다.
#
# Wayland 에서는 `--class` 가 먹지 않고 크롬이 URL 로 클래스를 만든다.
#   chrome-zzozzozzo.fly.dev__away-Default
# 프로필 이름(-Default)까지 붙으니 부분 일치로 잡는다.
WINDOW_CLASS="${AWAY_DESKTOP_WMCLASS:-zzozzozzo.fly.dev__away}"
...
remove_rule() {
  ...
  for key in above aboverule skiptaskbar skiptaskbarrule skipswitcher \
             skipswitcherrule skippager skippagerrule title titlematch \
...
kw --group "$RULE_ID" --key wmclass "$WINDOW_CLASS"
kw --group "$RULE_ID" --key wmclasscomplete false
kw --group "$RULE_ID" --key wmclassmatch 2
